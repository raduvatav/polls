...
function isDateSelected(year, month, day, remove) {
	...
	// remove selected date from calendar

	for (var i = 0; i < g_selected_dates.length; i++) {
		if ((g_selected_dates[i].year == year) && (g_selected_dates[i].month == month) && (g_selected_dates[i].day == day)) {
			if (remove) {
